...
const Contact = ({ t }) => {
  ...
  const sendViaWhatsApp = () => {
    if (!formData.message) {
      setModalData({ show: true, title: "Missing Info", message: "Please write an issue or comment before sending." });
      return;
    }
  ...
  const sendViaEmail = () => {
    if (!formData.email || !formData.message) {
      setModalData({ show: true, title: "Missing Info", message: "Please provide your email and issue/comment." });
      return;
    }
    ...
    <div className="bg-white py-16 min-h-screen fade-in relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 slide-up">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-rmc-black mb-2" dangerouslySetInnerHTML={{ __html: t("head_contact") }}></h2>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="relative w-full h-80 bg-gray-200 rounded-xl overflow-hidden shadow-inner mb-8">
              {mapLoading && (
                <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10">
                  ...
                  <p className="text-sm font-semibold text-rmc-dark-green mt-2">Finding nearest Masjids...</p>
                </div>
              )}
              <iframe 
                src={mapUrl}
              ...
              >
                <i className="fas fa-location-arrow mr-2"></i> Near Me
              </button>
            </div>
          ...
          <div>
            <div className="bg-rmc-dark-green rounded-xl shadow-lg p-1 text-center">
              ...
              >
                <i className="fas fa-envelope mr-3"></i> Mail Us 
            ...
            <div className="mail-us-dropdown bg-white shadow-xl rounded-b-xl border border-gray-200 mx-2 -mt-2 relative" style={{ maxHeight: isMailUsOpen ? "600px" : "0px" }}>
              <div className="p-6 pt-8">
                ...
                <div className="mb-6">
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Your Issue/Comment</label>
                ...
                <div className="flex flex-col sm:flex-row gap-4 border-t border-gray-100 pt-6">
                  <button type="button" onClick={sendViaWhatsApp} className="flex-1 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-lg transition flex items-center justify-center shadow">
                    <i className="fab fa-whatsapp text-xl mr-2"></i> Send Via WhatsApp
                  </button>
                  <button type="button" onClick={sendViaEmail} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 rounded-lg transition flex items-center justify-center shadow">
                    <i className="fas fa-paper-plane text-xl mr-2"></i> Send Via Email
                  </button>
                </div>
              </div>
            </div>
